...
def pivot_points_grid(points: DataFrame, buy_point: float = 2, sell_point: float = 2, type="中间值"):
    """枢轴点网格策略
    ...
    """
    cur_price = points.loc["*当前>", type]
    buy_price = points.loc[f"支撑位{buy_point}", type]
    sell_price = points.loc[f"阻力位{sell_point}", type]
